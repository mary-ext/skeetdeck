/**
 * @file
 * Graphemer, licensed under MIT License
 * Taken from commit ed86fe916dad2d90407c66fc6182d3c40f7fc827
...
export const countGraphemes = (str: string): number => {
	let count = 0;
	let index = 0;
	let brk;
 ...
 * @returns {number}
 */
const codePointAt = (str: string, idx: number): number => {
	if (idx === undefined) {
		idx = 0;
	}
...
const shouldBreak = (
	...
	const allEmoji = [startEmoji, ...midEmoji, endEmoji];
	...
	if (
		rIIndex > 0 &&
		all.slice(1, rIIndex).every((c) => c === CLUSTER_BREAK.REGIONAL_INDICATOR) &&
		[CLUSTER_BREAK.PREPEND, CLUSTER_BREAK.REGIONAL_INDICATOR].indexOf(previous) === -1
	) {
	...
	if (
		...
		all.slice(previousNonExtendIndex + 1, -2).every((c) => c === CLUSTER_BREAK.EXTEND) &&
...
const nextBreak = (string: string, index: number) => {
	if (index === undefined) {
		index = 0;
	}
	...
	const prevCP = codePointAt(string, index);
...
const getGraphemeBreakProperty = (code: number): number => {
...
const getEmojiProperty = (code: number): number => {
